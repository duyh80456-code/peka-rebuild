...
from scFoundation.model.pretrainmodels.select_model import select_model
...
class scFoundation_embedder(scLLM_QC_preprocess):
    ...
    def model_related_embed_infer_step(self,
                                       adata,
                                        ...
                                        use_amp = True,    # autocast FP16 to halve memory on small GPUs
                                        ):
        ...
        #Inference
        for i in tqdm(range(n_spots), desc=f" 🧬 scFoundation embedding ({n_spots} spots)",
            ...
            with torch.no_grad(), amp_ctx:
                ...
                if max_num >2:
                    ...
                    if pool_type=='all':
                        ...
                        raise ValueError('pool_type must be all or max')
                    geneexpemb.append(geneembmerge.detach().cpu().numpy())
                # Free per-spot intermediates aggressively (small-GPU OOM mitigation).
                if i % 64 == 0 and torch.cuda.is_available():
