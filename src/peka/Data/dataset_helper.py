...
class BatchLocalityDataset(Dataset):
    def __init__(self, dataset_save_folder, scLLM_emb_name,
        ...
        self.include_slides = set(include_slides) if include_slides is not None else None

        # Collect Anndata files (.h5ad)
        self.anndata_files = sorted(
            os.path.join(anndata_folder, f)
            for f in os.listdir(anndata_folder)
            if f.endswith('.h5ad') and (
                self.include_slides is None or os.path.splitext(f)[0] in self.include_slides
            )
        )
        ...
        if self.include_slides is not None:
            found = {os.path.splitext(os.path.basename(f))[0] for f in self.anndata_files}
            missing = self.include_slides - found
            if missing:
                raise ValueError(f"Requested slides not found in {anndata_folder}: {sorted(missing)}")
        ...
        if self.include_slides is not None and set(common_fname) != self.include_slides:
            missing = self.include_slides - set(common_fname)
            raise ValueError(f"Requested slides lack paired embeddings: {sorted(missing)}")
